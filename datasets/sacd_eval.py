...
from util.misc import (all_gather, get_rank, get_world_size,
                       is_dist_avail_and_initialized, is_main_process)


class sacd_evaluator(object):
    ...
    def update(self, batchresults, batchgt):
        ...
        for image_order, (image_id, pred_dict) in enumerate(batchresults.items()):
            ...
            gt_boxes_current = batchgt[image_order]['crop_box']
            ...
            top_5_iou_mat = iou_mat_current[:1]
            max_iou = top_5_iou_mat.max()
    ...
    def post_process_gt(self, batchgt):
        ...
        for datadict in batchgt:
            ...
            norm_factor = torch.tensor([w, h, w, h], dtype=torch.float32).to(datadict['crop_box'].device)
            ...
            minscale = torch.as_tensor([0., 0., 0., 0.]).to(datadict['crop_box'].device)
            maxscale = torch.as_tensor([1., 1., 1., 1.]).to(datadict['crop_box'].device)

            # datadict['boxes'] = datadict['boxes'] * (maxscale - minscale) + minscale
            datadict['crop_box'] = datadict['crop_box'] * (maxscale - minscale) + minscale

            # datadict['boxes'] = box_ops.box_cxcywh_to_xyxy(datadict['boxes'])
            datadict['crop_box'] = box_ops.box_cxcywh_to_xyxy(datadict['crop_box'])

            # datadict['boxes'] = datadict['boxes'] * norm_factor
            datadict['crop_box'] = datadict['crop_box'] * norm_factor
    ...
    def iou_cal(self):
        ...
        # 将iou_list中的值转换为张量
        iou_tensor = torch.tensor(self.iou_list, device=self.device)

        if is_dist_avail_and_initialized():

            dist.all_reduce(iou_tensor, op=ReduceOp.SUM)
            total_samples = torch.tensor(len(self.iou_list), device=self.device)
            dist.all_reduce(total_samples, op=ReduceOp.SUM)
            self.avg_iou = iou_tensor.sum() / total_samples.item()

        else:
            # 如果不是分布式环境，只在当前GPU计算avg_iou
            self.avg_iou = iou_tensor.mean().item()
